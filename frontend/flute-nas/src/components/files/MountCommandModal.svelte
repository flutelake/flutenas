<script lang="ts">
	...
	let mountCommands: Record<string, string> = {};
	$: mountCommands = currentShare
		? generateMountCommands(currentShare, currentServerIP, currentNFSVersion)
		: {};
		...
		{#if share}
			...
			<div class="mb-4">
				...
				<p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
					{selectedNFSVersion === 'nfs3'
						? 'NFS v3: Traditional, widely supported, requires portmapper'
						: selectedNFSVersion === 'nfs4'
							? 'NFS v4: Modern, stateful, better security'
					...
					{#each Object.keys(mountCommands) as tab}
						<li class="mr-2">
							<button
								...
								class="inline-block rounded-t-lg p-4 {activeTab === tab
									? 'active border-b-2 border-blue-600 text-blue-600 dark:border-blue-500 dark:text-blue-500'
									: 'text-gray-500 hover:border-gray-300 hover:text-gray-600 dark:text-gray-400 dark:hover:text-gray-300'}"
								type="button"
							>
								{getTabLabel(tab)}
							</button>
			...
			<Alert color="blue" class="mb-4">
				...
				<ul class="mt-2 list-inside list-disc text-sm">
					...
					<li>
						Create the mount directory if it doesn't exist (e.g., <code
							>sudo mkdir -p /mnt/{share.Name}</code
						>)
					</li>
