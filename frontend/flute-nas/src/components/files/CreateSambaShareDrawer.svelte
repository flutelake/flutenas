<script lang="ts">
	...
	function getSambaUsers() {
		...
		api
			.post('/v1/samba-user/list', { HostIP: CurrentHostIP })
			.then((resp) => {
...
>
	<div class="flex items-center">
		...
		>
			<InfoCircleSolid class="me-2.5 h-5 w-5" />Create Samba Share
		</h5>
	...
	<form action="#" class="mb-6">
		...
		<div class="mb-6">
			...
			<button on:click={() => (selectDirModalFlag = true)}
				><Input id="Pseudo" name="Pseudo" value={selectedPath} required placeholder="" /></button
			>
		</div>
		<!-- choose samba user -->
		<div class="mb-6">
					...
					<div class="col-span-1">
						{#if index == 0}
							<Button
								color="red"
								class="w-full"
								on:click={() => onClickDelUserInput(index)}
								disabled
							>
...
<DirectoryDialog
	...
	bind:selectedPath
	on:refresh_samba_user_list_msg={() => console.log('1112')}
></DirectoryDialog>
