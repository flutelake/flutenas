...
<Modal title="Upload Files" bind:open autoclose outsideclose>
	<Fileupload id="multiple_files" multiple bind:files />
	<div
		class="flex h-32 min-h-fit items-center justify-center rounded-md bg-gray-100"
		...
		{#if selectFiles}
			{#each selectFiles as item, index}
				<li
					...
					style="background-image: linear-gradient(to right, #6ee7b7 0%, #6ee7b7 {item.progress}%, #e0e7ff {item.progress +
						20}%, #e0e7ff 100%);"
				>
					{item.file.name}
				</li>
