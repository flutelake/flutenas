<script lang="ts">
	...
	import { CloseButton, Frame } from 'flowbite-svelte';
	import type { ColorVariant, ToastPositionType } from 'flowbite-svelte';
	...
	export let divClass: string = 'gap-3';
...
{#if toastStatus}
	<div
		class="fixed right-6 top-6 z-50 w-full min-w-80 max-w-xs max-w-xs bg-white p-4 text-gray-500 shadow dark:bg-gray-800 dark:text-gray-400"
	>
		<div role="alert" {...$$restProps} class={finalDivClass} transition:transition={params}>
			{#if $$slots.icon}
				<div class={iconClass}>
					<slot name="icon" />
				</div>
