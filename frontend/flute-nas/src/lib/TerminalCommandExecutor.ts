...
export class TerminalCommandExecutor {
    ...
    private processNextCommand = (): void => {
        if (this.commandQueue.length > 0 && !this.commandExecuting) {
            ...
            if (nextCommand) {
                ...
                this.options.socket.send("0:" + unescape(encodeURIComponent(nextCommand + '\n')).length.toString() + ":" + nextCommand + '\n');
    ...
    public handleOscSequence(sequence: string): void {
        if (sequence.includes('133;A')) {
            ...
            if (exitStatus !== 0) {
                ...
                this.options.term.writeln(`\x1b[1;31mCommand failed with exit status: ${exitStatus}. Stopping further execution.\x1b[0m`);
    ...
    private checkAllCommandsCompleted(): void {
        ...
        if (this.commandQueue.length === 0 && !this.commandExecuting) {
            ...
            this.options.term.writeln('\x1b[1;36mYou can now continue with manual commands if needed.\x1b[0m');
