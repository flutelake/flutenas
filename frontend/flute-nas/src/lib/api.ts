...
export class FluteAPI {
	...
	async getHostSystemInfo(hostIP: string = '127.0.0.1'): Promise<any> {
		return new Promise((resolve, reject) => {
			axios
				.get(`/v1/host/system-info?HostIP=${hostIP}`)
	...
	// 获取主机监控指标
	async getHostMonitoringMetrics(hostIP: string = '127.0.0.1'): Promise<HostMonitoringResponse> {
		return new Promise((resolve, reject) => {
			axios
				.get(`/v1/host/monitoring?HostIP=${hostIP}`)
				.then((resp: AxiosResponse) => {
					if (resp.data.code === 0) {
						resolve(resp.data.data as HostMonitoringResponse);
					} else {
						reject(new Error('Error code: ' + resp.data.code));
					}
				})
	...
	async queryVictoriaMetricsRange(
		req: VictoriaQueryRangeRequest
	): Promise<VictoriaQueryRangeResponse> {
		const resp = await this.post('/v1/metrics/query_range', req);
		return resp.data as VictoriaQueryRangeResponse;
	}
